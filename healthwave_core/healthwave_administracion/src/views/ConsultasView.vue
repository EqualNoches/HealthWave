<template>
  <div class="border-round-lg border-1 p-2 card h-full">
    ...
    >
      ...
      <Column
        field="consultaCodigo"
        header="#"
        style="width: 5%"
        sortable
      ></Column>
      <Column
        field="documentoPaciente"
        header="Documento del Paciente"
        sortable
      ></Column>
      <Column
        field="documentoMedico"
        header="Documento del Médico"
        sortable
      ></Column>
        ...
        <template #body="slotProps">
          <div class="flex justify-content-center gap-2">
            ...
            <Button
              icon="pi pi-trash"
              severity="danger"
              @click="ConfirmarEliminar($event, slotProps.data.consultaCodigo)"
              type="button"
              text
            />
          </div>
    ...
    <Dialog
      ...
      modal
      :header="modoEdicion ? 'Editar consulta' : 'Nueva consulta'"
      :style="{ width: '50vw' }"
    ...
    <DatePicker v-model="model.fecha" 
                dateFormat="yy-mm-dd"
                :showIcon="true"
                inputId="fecha"
                class="w-full" />
        ...
        <div class="col-12 md:col-6">
          ...
          >
          <InputText v-model="model.idAutorizacion" class="w-full" />
        </div>
        <div class="col-12 md:col-6">
    ...
    async Guardar() {
      try {
        const response = await api[this.modoEdicion ? "put" : "post"](
          `api/Consulta/${this.modoEdicion ? "update" : "add"}`,
          this.model
        );
        ...
        push.error("Ha ocurrido un error al guardar la consulta");
      }
    },
    async Eliminar(consultaCodigo) {
      const response = await api.delete(
        `api/Consulta/delete/${consultaCodigo}`
      );
      if (response.status === 200) {
